...
if ($method == 'GET') {
    try {
        if ($isAdmin) {
            ...
            $query = "SELECT * FROM offers WHERE is_active = 1 ORDER BY id DESC";
        }
    ...
    try {
        $stmt = $conn->prepare("UPDATE offers SET title=?, description=?, code=?, discount_percentage=?, image_url=?, is_active=?, valid_until=? WHERE id=?");
        $stmt->execute([
            $data['title'],
            $data['description'],
            isset($data['code']) ? $data['code'] : null,
